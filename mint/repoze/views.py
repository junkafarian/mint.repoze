...
from mint.repoze.models import videos, get_videos_with_tag_html

@bfg_view(name='index')
def index(context, request):
    context.video_name = 'intro'
    context.video = videos[context.video_name]
    return render_template_to_response('templates/index.html', context=context)

@bfg_view(name='video')
def video(context, request):
    if context.video_name not in videos:
...
def tag(context, request):
    context.videos = get_videos_with_tag_html(context.tag)
